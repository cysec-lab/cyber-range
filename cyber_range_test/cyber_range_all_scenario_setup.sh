    ...
    for num in ${client_nums[@]}; do
        ...
        if [ "$clone_type" = 'zfs' ]; then
            if [ $scenario_num -eq 3 ]; then
            	...
            	$tool_dir/zfs_clone_vm.sh $num ${CLIENT_TEMP_NUMS[$scenario_num]} $_hostname $group_network_bridge
            fi
            ...
            if [ $scenario_num -eq 2 ]; then
            	...
            	$tool_dir/clone_vm.sh $num ${CLIENT_TEMP_NUMS[$scenario_num]} $_hostname $TARGET_STRAGE $group_network_bridge
            fi
            if [ $scenario_num -eq 1 ]; then
                $tool_dir/change_format.sh $num
                $tool_dir/centos_config_setup.sh $num $ip_address $_hostname # change cloned vm's config files
